...
const copy = {
  fr: {
    ...
    locationBody2: "Le fond de carte, lui, est celui d'Apple Plans, servi par iOS (MapKit). C'est la seule requête réseau que l'application émette, elle n'a lieu que sur les écrans de course, et elle transmet à Apple la zone géographique affichée — pas ton identité, pas ton tracé, qui est dessiné par-dessus sur l'appareil. Tu peux la désactiver dans le profil : la carte affiche alors ton parcours seul.",
    watch: "L'Apple Watch",
  ...
  en: {
    ...
    locationBody2: "The map background, however, is Apple Maps, served by iOS (MapKit). It is the only network request the app makes, it only happens on the running screens, and it transmits to Apple the geographic area being displayed — not your identity, not your route, which is drawn on top on the device. You can turn it off in the profile: the map then shows your route alone.",
  ...
  es: {
    ...
    locationBody2: "El fondo de mapa, en cambio, es el de Apple Plans, servido por iOS (MapKit). Es la única petición de red que hace la aplicación, solo ocurre en las pantallas de carrera y transmite a Apple la zona geográfica mostrada, no tu identidad ni tu traza, que se dibuja encima en el dispositivo. Puedes desactivarlo en el perfil: el mapa muestra entonces solo tu recorrido.",
